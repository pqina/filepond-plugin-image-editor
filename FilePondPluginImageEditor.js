...
var FilePondPluginImageEditor = (function () {
    ...
    const plugin = (_) => {
        ...
        addFilter('DID_CREATE_ITEM', (item, { query, dispatch }) => {
            ...
            if (color)
                item.setMetadata('colors', item.getMetadata('color'));
            item.extend('edit', () => {
        ...
        const createImagePoster = (query, item, done = () => { }) => {
            ...
            if (!query('GET_FILE_POSTER_FILTER_ITEM')(item))
                return done();
            const { imageProcessor, imageReader, imageWriter, editorOptions, legacyDataToImageState, imageState: imageBaseState, } = getEditorSafe(query('GET_IMAGE_EDITOR'));
